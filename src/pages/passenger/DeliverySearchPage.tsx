...
/** Modelée sur SearchDestinationPage.tsx (même contrat de navigation/UX) sans y toucher — un point de collecte de livraison est conceptuellement un second champ que le flux course n'a pas. */
...
function PlaceRow({ icon, title, subtitle, onClick }: { icon: React.ReactNode; title: string; subtitle: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary-50 text-primary-800">{icon}</div>
...
export default function DeliverySearchPage() {
  ...
  const selectDropoff = (id: string) => {
    navigate('/passenger/delivery/booking', { state: { dropoffId: id } });
  };

  const selectSavedPlace = (place: SavedPlace | undefined) => {
    const neighborhoodId = resolveNeighborhoodId(place);
    if (neighborhoodId) selectDropoff(neighborhoodId);
  ...
  return (
    <div className={cn('mx-auto min-h-screen max-w-md bg-background px-5 pb-10 pt-6', 'lg:max-w-xl')}>
      ...
      <p className="mb-5 font-display text-h2 text-foreground">Où livrer le colis ?</p>
      ...
      <div className="mb-4 flex items-center gap-3 rounded-md border border-input bg-surface px-4 py-3">
        ...
        <div className="min-w-0">
          <p className="text-caption text-muted-foreground">Collecte</p>
          <p className="truncate text-body-sm font-medium text-foreground">{getNeighborhood(PICKUP_ID)?.name}, Conakry</p>
        </div>
      </div>
      ...
      {isTyping ? (
        ...
        <div className="mt-4 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-1">
              {RECENT_IDS.map((id) => {
                ...
                if (!n) return null;
                return <PlaceRow key={id} icon={<Clock className="h-4 w-4" />} title={n.name} subtitle="Conakry" onClick={() => selectDropoff(id)} />;
              })}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1">
              {NEIGHBORHOODS.map((n) => (
                <PlaceRow key={n.id} icon={<MapPin className="h-4 w-4" />} title={n.name} subtitle="Conakry" onClick={() => selectDropoff(n.id)} />
              ))}
            </div>
          </div>
        </div>
